...
function Relationclientcc ($Relationclient){
	...
	function modifierRelationclient($Relationclient,$email){
		$sql="UPDATE Relationclient SET nom=:nom,prenom=:prenom,type=:type,email=:email,adresse=:adresse,gouvernorat=:gouvernorat,ville=:ville,codePostal=:codePostal,telephone=:telephone, password=:password WHERE email=:email";
...
try{		
        ...
        $telephone=$Relationclient->getTelephone();
        $password=$Relationclient->getPassword();
		$datas = array(':nom'=>$nom,':prenom'=>$prenom,':type'=>$type, ':email'=>$email,':adresse'=>$adresse, ':ville'=>$ville,':codePostal'=>$codePostal,':telephone'=>$telephone,':password'=>$password);
		$req->bindValue(':nom',$nom);
		$req->bindValue(':prenom',$prenom);
