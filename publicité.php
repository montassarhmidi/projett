<?php  include "core/publiciteC.php";
			...
			<div class="container">
				...
				<div id="responsive-nav">
					...
					<ul class="main-nav nav navbar-nav">
						<li class="active"><a href="publicité.php">Acceuil</a></li>
...
<select name="type" style="width: 300px; height: 40px;font-size: 20px;  border-radius: 50px;background:rgba(230, 230, 230, 0.3);">
 <option  value="Tous Les Types">Afficher tous les Pubs</option> 
...
<br><br>
<input type="submit" name="submit" value="Rechercher" style="width: 200px; height: 40px; background-color: #D10024;
...
{
	if($_POST['type']=='Accessorie Informatique')
	{
		$listepublicite=$PublicC->tribytype($_POST['type']);

	}
	else if($_POST['type']=='Materiel Informatique')
	{
		$listepublicite=$PublicC->tribytype($_POST['type']);

	}
	else if($_POST['type']=='Materiel Sucurite')
	{
		$listepublicite=$PublicC->tribytype($_POST['type']);
...
$pourcentage=round(((($row['prix_sans_remise']-$row['prix_avec_remise'])*100)/($row['prix_sans_remise'])));
										...
										<div class="product">
											...
											<a href="singlepage.php?id=<?php echo $row['id']?>"><button  class="add-to-cart-btn"><i ></i> consulter produit</button></a>
										</div>
