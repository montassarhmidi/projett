...
function Relationclientcc ($Relationclient){
		...
		echo "Type: ".$Relationclient->getType()."<br>";
	...
	function ajouterRelationclient($Relationclient){
		$sql="insert into relationclient (Nom,Prenom,Type,Email,Adresse,Gouvernorat,Ville,CodePostal,Telephone,Password) values (:Nom,:Prenom,:Type,:Email,:Adresse,:Gouvernorat,:Ville,:CodePostal,:Telephone,:Password)";
	...
	function modifierRelationclient($Relationclient,$id){
		$sql="UPDATE Relationclient SET Nom=:Nom,Prenom=:Prenom,Type=:Type,Email=:Email,Adresse=:Adresse,Gouvernorat=:Gouvernorat,Ville=:Ville,CodePostal=:CodePostal,Telephone=:Telephone,Password=:Password WHERE id=:id";
...
try{		
		...
		$datas = array(':Nom'=>$Nom,':Prenom'=>$Prenom,':Type'=>$Type,':Email'=>$Email,':Adresse'=>$Adresse,':Gouvernorat'=>$Gouvernorat,':Ville'=>$Ville,':CodePostal'=>$CodePostal,':Telephone'=>$Telephone,':Password'=>$Password);
...
function triclient($recherche){
		
		$sql="SELECT * FROM Relationclient WHERE Type='$recherche'";
		$db = config::getConnexion();
		try{
		$liste=$db->query($sql);
		return $liste;
